...
| Path | Package | Role |
|------|---------|------|
| `norn/` | `norn` | the library: engine (cards, deal, shuffle, predeal, smartstack, render, generate) + generic bridge evaluation over the `HandSummary` index (`summary.odin`). No `main` — imported by everything else. |

...
/*
	...
	whole `norn.Deal` (rather than a single `norn.Hand`) because they reason about both sides — they
...
is_1major_overcall :: proc(hand: norn.Hand) -> bool {
...
is_1d_takeout :: proc(hand: norn.Hand) -> bool {
	if norn.hcp(hand) < 11 {
		return false
	}
...
has_both_majors_michaels :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
north_south_may_overcall_1N :: proc(board: norn.Deal, top_nt_range: int) -> bool {
	north := board[.North]
	east := board[.East]
	south := board[.South]
	west := board[.West]
...
south_may_overcall_opponents_1N_with_north_invitational :: proc(board: norn.Deal, top_nt_range: int) -> bool {
	north := board[.North]
	east := board[.East]
	south := board[.South]
	west := board[.West]

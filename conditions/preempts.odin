...
is_any_weak_6_plus_carder :: proc(hand: norn.Hand) -> bool {
...
is_any_weak_or_min_7_plus_carder :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
is_generic_weak2d :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
is_generic_5card_unbal_weak2 :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
	if points < 6 || points > 10 {
		return false
	}
...
is_weak2_5card_major :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
is_weak2_major :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
	if points < 6 || points > 11 {
		return false
	}
...
is_weak_5_or_6_card_major :: proc(hand: norn.Hand) -> bool {
...
is_semi_positive_weak_two_hearts :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
is_semi_positive_majors_two_suiter :: proc(hand: norn.Hand) -> bool {
	if !two_suiter(hand) {
		return false
	}
...
is_gf_majors_two_suiter :: proc(hand: norn.Hand) -> bool {
	if !two_suiter(hand) {
		return false
	}
	if norn.heart_length(hand) < 5 || norn.spade_length(hand) < 5 {
...
is_gf_hearts_minor_two_suiter :: proc(hand: norn.Hand) -> bool {
	if !two_suiter(hand) {
		return false
	}
	hs := norn.heart_length(hand)
...
is_minors_2n_preempt :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
	if points < 6 || points > 11 {
		return false
	}
...
is_shapely_minor_preempt :: proc(hand: norn.Hand) -> bool {
	if norn.hcp(hand) > 10 {
		return false
	}
...
is_likely_3major_preempt :: proc(hand: norn.Hand) -> bool {
	if norn.hcp(hand) > 10 {
		return false
	}
	ss := norn.spade_length(hand)
...
any_offensive_suit :: proc(hand: norn.Hand, offense_tricks: int) -> bool {
	return(
...
is_likely_4level_preempt :: proc(hand: norn.Hand) -> bool {
	if norn.hcp(hand) > 12 {
		return false
	}
...
is_insane_offensive_preempt :: proc(hand: norn.Hand) -> bool {
	if norn.hcp(hand) > 13 {
		return false
	}
...
is_8_plus_tricks :: proc(hand: norn.Hand) -> bool {
	if norn.hcp(hand) < 14 {
		return false
	}
...
is_powerhouse :: proc(hand: norn.Hand, maxlosers: int) -> bool {
...
is_weak2_major_in_range :: proc(hand: norn.Hand, low, high: int) -> bool {
	points := norn.hcp(hand)

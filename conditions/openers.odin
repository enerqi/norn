...
// `is_any_1c_opener`.)
is_any_1c_opener :: proc(hand: norn.Hand) -> bool {
...
is_1d_unbal_opener :: proc(hand: norn.Hand) -> bool {
	if is_flattish(hand) {
		return false
	}
...
is_1d_opener :: proc(hand: norn.Hand) -> bool {
...
is_1major_opener :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
	if points < 11 || points > 15 {
		return false
	}
...
is_light_1major_opener :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
is_marmic :: proc(hand: norn.Hand) -> bool {
...
is_2d_opener :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
is_2d_intermediate_opener :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
is_3n_opener :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
opens_std_1minor_prepared :: proc(hand: norn.Hand) -> bool {
	points := norn.hcp(hand)
...
is_3cd_opener_1st2nd :: proc(hand: norn.Hand) -> bool {
	if norn.hcp(hand) > 11 {
		return false
	}
...
is_standard_3cd_7carder :: proc(hand: norn.Hand) -> bool {
	if norn.hcp(hand) > 11 {
		return false
	}
	if has_side_major(hand) {

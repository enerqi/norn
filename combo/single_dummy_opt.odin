...
// Public: this IS combo's blind recommendation for a holding, so callers comparing it against an external
// source — e.g. a `Suit_Book` provider diffing published percentages against ours — can ask for it directly.

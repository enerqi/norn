...
// order (s,h,d,c). For each suit it names the blind single-dummy line to play — the registered book's line
...
// Stop and free the persistent pool + every worker's memo maps, plus whatever the registered suit book built. Optional — safe to skip (the OS reclaims it all
...
// When the registered published suit-combination table (see book.odin) covers this holding, the card page
// OVERRIDES its engine recommendation with the book's: `label` is the book's play (the cell label), `tip` is
// that play plus the book's make-percentages vs best defence (the hover tooltip). `ok` is false on a miss —
// including when no book is registered at all — so the caller keeps the engine line + narration. The book
// fixes the holdings where our fixed lines underperform published theory.
@(private)
book_override :: proc(
	n, s: u16,
	...
	if g_book.lookup == nil {
		return
	}
	e, hit := g_book.lookup(n, s)
	if !hit {
		return
	}
	entry = e
...
// Build a per-trick-count probability row from a book entry's CUMULATIVE targets (each = P(>= need)
...
@(private)
book_pk_row :: proc(e: Book_Entry) -> (row: [RANKS + 1]f64) {
	cum: [RANKS + 2]f64
	listed: [RANKS + 2]bool
	max_need := 0
	for i in 0 ..< e.nt {
...
// One partnership's per-suit published-book override (label + tooltip), in DISPLAY_SUITS order. Computed ONCE
// per partnership and shared by the label and tooltip emit: each entry costs a registered-book lookup (whose
// `book_key` runs `sd_best_joint_table`), so precomputing halves that work vs the label and tip paths each
...
suit_overrides :: proc(
	...
	for suit, i in DISPLAY_SUITS {
		e: Book_Entry
		out[i].label, out[i].tip, e, out[i].ok = book_override(
...
// Write the per-suit published (book) per-trick shape as a JSON array `[<[p0..p13]>|null, ...]` in
...
// book's line where it covers the holding (an OVERRIDE, see `book_override`), else the best

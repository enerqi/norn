...
cost lands on allocation-heavy and multi-threaded code. Deal generation is exactly that shape - a
`--count 10000` run allocates per accepted deal and the generator pools threads - so the release
default below matters here rather than being a theoretical saving.

The default follows `-debug`, which is the same line the backtrace symbols fall on: diagnostics in
the builds you debug, nothing in the builds you measure. Override it in either direction;
